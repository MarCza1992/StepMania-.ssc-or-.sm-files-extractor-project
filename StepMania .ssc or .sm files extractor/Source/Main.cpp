			...
			for (const auto& ChartFolder : filesystem::directory_iterator(ChartsPack))
			{
				if (filesystem::is_directory(ChartFolder))
				{
					...
					for (const auto& file : filesystem::directory_iterator(ChartFolder))
					{
						...
						if (file.path().extension() == ".sm")
						{
							smpath = file.path().string();
						}
					}
						...
						while (getline(plik, linia))
						{
							...
							if (linia.find("#NOTES:") == 0)
							{
								...
								while (getline(plik, linia))
								{
									...
									if (linia.find("measure") != string::npos)
									{
										continue;
									}
								...
								{
									chart.charter = chart.technotation;
									chart.technotation = " ";
								}

								if (chart.charter == chart.technotation)
								{
									chart.technotation = " ";
								}
					...
					else
					{
						...
						while (getline(plik, linia))
						{
							...
							if (linia.find("//---") == 0)
							{
								...
								if (chart.charter.empty())
								{
									size_t start = linia.find("//---") + 32;
									size_t end = linia.length() - 16;
									chart.charter = linia.substr(start, end - start);
								}
							}
